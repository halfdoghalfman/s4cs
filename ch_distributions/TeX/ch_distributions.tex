...
\[
	f_X(x) = \frac1{\sqrt{2\pi}\cdot\sigma} e^{-((x-\mu)/\sigma)^2/2},\quad -\infty<x<\infty.
\]
See~\Cref{simpleNormal}.
...
Let us see how to reproduce the values in~\Cref{zTableShort} using Google Sheets.
...
\end{eqnarray*}
is read \term{n choose k}.\footnote{Other notation for $n$ choose $k$ includes $_nC_k$, $C_n^k$, and $C(n,k)$.} The exclamation point notation (e.g. $k!$) denotes a \term{factorial}\label{factorialDefinitionInTheBinomialSection} expression (\Cref{factorialTable}).
...
\[
	\{1\}, \{2\}, \{3\}, \{4\}.
\]
There is exactly one 0-element subset of [4],
\[
	\emptyset,
\]
so $\binom40=1$.
Next, $\binom43=4$ because there are exactly four 3-element subsets of [4]:
\[
...
When drawing a sample from a larger population, the \term{10\% condition} helps our samples to be independent. If our sample has more than 10\% of the individuals, then knowing that 5\% has a certain property may mean that the other 5\% cannot have this property. See also \Cref{tenpercent}.
...
The Poisson distribution is a popular model for risky events studied by actuaries.
Specifically, this is so in the case where we know how many events happen on average,
but we do not have values of $n$ and $p$ to set up a binomial or normal distribution.
Click on the camera icon above for a video showcasing Guided Practice \ref{doris_kung} and featuring an author of the present text.
...
The following example may come up in playing with kindergarteners.
Suppose there are five cards, two of which shows hair clips, and one each show scissors, hairbrush, and comb.
...
\begin{example}{Earned premium.}\label{drc}
	...
	As the months go by, the insurance company considers that it is earning the premium linearly:
...
\begin{example}{Earned premium, two policies.}
	...
	Solution:
		...
		There are four cases, see \Cref{chatGPT}.
		% $F_1\cap F_2$,
		% $F_1\setminus F_2$,
		% $F_2\setminus F_1$,
		% $\bar{F_1}\cap\bar{F_2}$.
...
\begin{tabular}{clll}
\hline
\textbf{Case} & \textbf{Event} & \textbf{Probability} & \textbf{Earned Premium} \\
\hline
1 & $F_1 \cap F_2$ & $(1-(1-p)^6)(1-(1-p)^3)$ & $2(100 - 100{,}000)$ \\
2 & $F_1 \setminus F_2$ & $(1-(1-p)^6)(1-p)^3$ & $100 + 25 - 100{,}000$ \\
3 & $F_2 \setminus F_1$ & $(1-p)^6(1-(1-p)^3)$ & $2(100 - 100{,}000)$ \\
4 & $\bar{F_1} \cap \bar{F_2}$ & $(1-p)^6(1-p)^3 = (1-p)^9$ & $75 + 25$ \\
\hline
\end{tabular}
\caption{Probabilities and earned premiums for each case.}\label{chatGPT}
\end{table}		
...
E(X)
  &= 2(100 - 100{,}000)(1-\beta^2)(1-\beta)
   + (100 + 25 - 100{,}000)(1-\beta^2)\beta \notag\\
  &\quad + 2(100 - 100{,}000)\beta^2(1-\beta)
   + (75 + 25)\beta^3 \notag\\[4pt]
  &= 2f(1-\beta) + (25 + f)(1-\beta^2)\beta + 100\beta^3 \notag\\
  &= 2f + (25 - f)\beta + (100 - 25 - f)\beta^3,
\end{align}
	...
	\begin{example}{CEO effort.}
		...
		\end{quote}
		If we take $\sigma=1$ and $v=1$, what effort $a_i$ is needed to ensure that with 95\% probability, the earnings are greater than 1?\footnote{We need $95\%=P(e_i>1)=P(a_i+\epsilon_i>1)=P(\epsilon_i>1-a_i)=1-\Phi(1-a_i)$.
        Thus, $\Phi(1-a_i)=1/20$ and $a_i=1-\Phi^{-1}(1/20)$.
	...
	\begin{example}{Principal's preferences}
		...
		\end{quote}
		\begin{quote}
		Each principal cares about her terminal payoff $V_i$ at date 5. The principal offers $w_i (r_i)$ to the agent to maximize the expected payoff:
		\[
